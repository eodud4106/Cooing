	...
	$.ajax({
		...
		success : function(array) {
			...
			$(array).each(function(i, data) {
				...
				str += '<img src="./resources/img/group3.png" width="30" height="30">' + data.party_name + '</p></div>';
			});		
			$('#group').html(str);
		},
		error : function(e) {
...
function AlbumListPaging_hindoong(check, result) {
	if (check)
		$('.card-columns').html('');
	var category = ['여행' , '스포츠 및 레저' , '동물' , '음악' , '음식 및 요리' , '패션 및 뷰티' , '연예 및 TV' , '게임' , '영화' , '도서' , '공연 및 전시' , '외국어' , '전문지식' , '수집 및 제작' , '자기계발' , '육아' , '일상생활' , '자동차' , '낚시' , '건강'];
